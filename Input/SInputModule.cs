...
[AddComponentMenu("Rewired/SInput Module")]
public sealed class SInputModule : RewiredPointerInputModule {
	...
	private void ProcessTouchPress(PointerEventData pointerEvent, bool pressed, bool released) {
		...
		Debug.Log($"aöowdaöodjwa " + currentOverGo.name);
	...
	private void HandleMouseTouchDeselectionOnSelectionChanged(GameObject currentOverGo, BaseEventData pointerEvent) {
		...
		if (selectHandlerGO != null) {
			// 1. Problem 2 gelöst: Ignoriere Klicks auf deaktivierte Elemente
			// (Selectable ist in UnityEngine.UI, also für beide Assemblies sichtbar)
			var selectable = selectHandlerGO.GetComponent<UnityEngine.UI.Selectable>();
			if (selectable != null && !selectable.interactable) {
				selectHandlerGO = null;
			}
			// 2. Problem 1 gelöst: Ignoriere Klicks auf Elemente, die explizit nicht selektierbar sind
			else {
				var selectabilityCheck = selectHandlerGO.GetComponent<ISelectabilityCheck>();
				if (selectabilityCheck != null && !selectabilityCheck.IsSelectable) {
					selectHandlerGO = null;
				}
			}
		}
	...
	private void SetUpRewiredPlayerMice() {
		...
		for (int i = 0; i < playerMice.Count; i++) {
			...
			if (Rewired.Utils.UnityTools.IsNullOrDestroyed(mouse)) continue;
