...
namespace PathFinding {
	...
	public class Graph {
		...
		public Path Find(Vector3 origin) {
			return Find(GetNearest(origin));
		}

		public Node GetNearest(Vector3 pos) {
			return Nodes.OrderBy(n => (pos - n.Position).sqrMagnitude).First();
		}

		public int GetNearestIndex(Vector3 pos) {
			return Nodes.IndexOf(Nodes.OrderBy(n => (pos - n.Position).sqrMagnitude).First());
		}
